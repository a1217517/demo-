...
//        String "jps -ml|grep org.apache.hadoop.hive.metastore.HiveMetaStore"
        ArrayList<String> command = new ArrayList<String>();

        //bin/spark-submit --class org.apache.spark.examples.SparkPi --master local  --deploy-mode client examples/jars/spark-examples_2.11-2.3.0.jar 1

      /*  command.add("/opt/beh/core/spark/bin/spark-submit");
        command.add("--class");
        command.add("org.apache.spark.examples.SparkPi");
        command.add("--master");
        command.add("yarn");
        command.add("--deploy-mode");
        command.add("client");
        command.add("/opt/beh/core/spark/examples/jars/spark-examples_2.11-2.3.0.jar");
        command.add("1");*/
        if (!fileIsExist("hdfs://beh/var/log/")){
            //在hdfs上创建日志文件
            command.add("hadoop");
            command.add("fs");
            command.add("-mkdir");
            command.add("hdfs://beh/var/log/hadoop-spark");
        }else {
            command.add("hadoop");
            command.add("fs");
            command.add("-ls");
...
//        command.add("-c");
//        command.add("/bin/echo $BEH_HOME");
//        command.add("");
//        command.add("jps -ml|grep org.apache.hadoop.hive.metastore.HiveMetaStore");
//        command.add("grep");
//        command.add("org.apache.hadoop.hive.metastore.HiveMetaStore");


//        command.add("tar");
//        command.add("xzf");
//        command.add("/Volumes/BACK/beh-7.5/hive-1.1.0-cdh5.7.0.tar.gz");
//        command.add("-C");
//        command.add("/Volumes/BACK/beh-7.5/");

        try {
            ShellCommandUtil.Result result = ShellCommandUtil.runCommand(command.toArray(new String[command.size()]));
            System.out.println(JSON.toJSONString(result));
          /*  //检查状态
            if (result.getStdout().contains("hadoop-spark") == true) {
                System.out.println("有hadoop-spark文件");
            }*/
        } catch (IOException e) {
            e.printStackTrace();
            System.out.println("shell命令执行异常,IOException");
        } catch (InterruptedException e) {
            e.printStackTrace();
            System.out.println("shell命令执行异常,InterruptedException");
        }
    }
    ...
    public static boolean fileIsExist(String fileName){
        ...
        try {
            ...
            //检查状态
            if (result.getStdout().contains("hadoop-spark") == false) {
                return false;
            }
        } catch (IOException e) {
            e.printStackTrace();
